...
// NewMemory memory模式
func NewMemoryCache() AdapterCache {
	return &MemoryCache{
		items: new(sync.Map),
	}
}
...
func (m *MemoryCache) Increase(key string) error {
	return m.calculate(key, 1)
}

func (m *MemoryCache) Decrease(key string) error {
	return m.calculate(key, -1)
}

func (m *MemoryCache) calculate(key string, num int) error {
	m.mutex.RLock()
...
func (m *MemoryCache) Expire(key string, dur time.Duration) error {
	m.mutex.RLock()
	defer m.mutex.RUnlock()
	item, err := m.getItem(key)
	if err != nil {
